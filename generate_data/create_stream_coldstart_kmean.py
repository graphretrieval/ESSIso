...
for max_subgraph_nodes in [10, 15, 20, 25, 30]:
    ...
    savedir = os.path.join(args.embedding_model, '{}_{}_coldstart'.format(args.prefix.split('/')[-1], args.max_subgraph_nodes))
    ...
    kmeans = KMeans(n_clusters=100).fit(mean_embedding_subgraphs)
    ...
    for i, subgraph in tqdm(enumerate(final_subgraph_queries), desc="gen embedding subgraph"):
        ...
        dimas_query_lines.append(create_dimas_graph_format(subgraph, sub_id_map, final_embedding_subgraphs[i], i, ori_graph_data.multi2single_label))
        query_lines += create_graph_format(subgraph, sub_id_map, final_embedding_subgraphs[i], i, ori_graph_data.multi2single_label)
