...
class Dataset:
    ...
    ## Load graph data
        ...
        if isinstance([node for node in G.nodes()][0], int):
        ...
        if class_map_file.is_file():
            ...
            all_labels = set([tuple(label) for label in self.class_map.values()])
            self.multi2single_label = {multi:single for single, multi in enumerate(all_labels)}
            if self.multiclass or 1:
                self.labels = [None]*len(self.class_map.keys())
                self.true_labels = np.zeros((self.feats.shape[0], 1), dtype=np.int64)
                for idx in self.class_map.keys():
                    self.labels[self.id_map[idx]] = self.class_map[idx]
                    self.true_labels[self.id_map[idx]] = self.multi2single_label[tuple(self.class_map[idx])]
            ...
            # self.num_class = len(self.class_map[[node for node in G.nodes()][0]])
            self.num_class = len(self.multi2single_label)
            print("File loaded successfully")
        else:
            print("Class map doesn't exist")
        
        ############################### FOR CLASS as FEATURE test #############################
        self.raw_feats = self.labels
    ...
    def preprocess_data(self, prefix, normalize=True, load_adj_dir = None, use_random_walks = True, load_walks=False, num_walk = 50, walk_len = 5, supervised=True, train_all_edge=False):
        ...
        self.nodes_ids = np.array([n for n in G.nodes.keys()])
