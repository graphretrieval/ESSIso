    ...
    for (Graph queryGraph : queryGraphVector) {
        ...
        if (arguments.useCache) {
            ...
            if (!arguments.brutal) {
                ...
                std::map<std::pair<int, std::vector<int>>, int > queryVector = queryGraph.wlFeatures;
                for (const auto & cachedGraph: graphCache.cachedGraphs) {
                    float distance = 0;
                    std::map<std::pair<int, std::vector<int>>, int > cachedVector = cachedGraph.second.wlFeatures;
                    std::map<std::pair<int, std::vector<int>>, int > temp = queryVector;
                    temp.insert(cachedVector.begin(), cachedVector.end());

                    for (const auto& key : temp) {
                        int valQuery = 0;
                        int valCached = 0;
                        std::map<std::pair<int, std::vector<int>>, int >::iterator it;
                        it = queryVector.find(key.first);
                        if (it != queryVector.end()) {
                            valQuery = it->second;
                        }
                        it = cachedVector.find(key.first);
                        if (it != cachedVector.end()) {
                            valCached = it->second;
                        }
                        distance += abs(valCached-valQuery);
                        // if (distance > bestDistance) break;
                    }
                    distanceMap.insert({cachedGraph.first, distance});
                    if (distanceQueue.size() < arguments.k) {
                        distanceQueue.push({distance, cachedGraph.first});
